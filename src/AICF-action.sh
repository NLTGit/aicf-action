...
function aicfApply { 
    ...
    # Install OPA bin
    curl -L -o opa https://github.com/open-policy-agent/opa/releases/download/v${INPUT_OPAVERSION}/opa_linux_amd64 && sudo chmod +x opa && sudo mv opa /usr/bin
    # Install regula script and libraries.
    sudo mkdir -p /usr/bin/regula && curl -L "https://github.com/fugue/regula/archive/v${INPUT_REGULAVERSION}.tar.gz" | sudo tar -xz --strip-components=1 -C /usr/bin/regula
    ...
    case "${INPUT_CLOUDPROVIDER}" in
        aws)
          opa eval --format pretty --input tfplan.json --data /usr/bin/regula/rego/lib --data /usr/bin/regula/rego/rules/tf/aws --data /usr/bin/regula/rego/examples/aws/useast1_only.rego --data waivers.rego 'data.fugue.regula.report' | sudo tee evaluate
          ;;
        gcp)
          opa eval --format pretty --input tfplan.json --data /usr/bin/regula/rego/lib --data /usr/bin/regula/rego/rules/tf/gcp --data waivers.rego 'data.fugue.regula.report' | sudo tee evaluate
          ;;
        azure)
          opa eval --format pretty --input tfplan.json --data /usr/bin/regula/rego/lib --data /usr/bin/regula/rego/rules/tf/azure --data waivers.rego 'data.fugue.regula.report' | sudo tee evaluate
          ;;
        *)
