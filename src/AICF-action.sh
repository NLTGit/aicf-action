...
function aicfApply { 
    ...
    case "${INPUT_CLOUDPROVIDER}" in
        aws)
          opa eval --format pretty --input tfplan.json --data ~/regula/rego/lib --data ~/regula/rego/rules/tf/aws --data ~/regula/rego/examples/aws/useast1_only.rego --data waivers.rego 'data.fugue.regula.report' | tee evaluate
          ;;
        gcp)
          opa eval --format pretty --input tfplan.json --data ~/regula/rego/lib --data ~/regula/rego/rules/tf/gcp --data waivers.rego 'data.fugue.regula.report' | tee evaluate
          ;;
        azure)
          opa eval --format pretty --input tfplan.json --data ~/regula/rego/lib --data ~/regula/rego/rules/tf/azure --data waivers.rego 'data.fugue.regula.report' | tee evaluate
          ;;
        *)
